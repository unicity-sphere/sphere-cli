  ...
  try {
    switch (command) {
      ...
      case 'invoice-list': {
        ...
        if (createdByMe) optionsMut['createdByMe'] = true;
        if (targetingMe) optionsMut['targetingMe'] = true;
        if (stateFilter) {
        ...
        if (limitIdx2 !== -1 && args[limitIdx2 + 1]) {
          ...
          if (!Number.isNaN(limit) && limit > 0) {
            optionsMut['limit'] = limit;
          }
        }
        const options = optionsMut as GetInvoicesOptions;
...
// Auto-run removed: legacyMain() is called from src/index.ts dispatcher.
